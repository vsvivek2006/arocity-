export const siteConfig = {
  name: 'Aerocity Escort Service',
  city: 'Aerocity, New Delhi',
  structuredAddress: {
      "streetAddress": "Asset Area 4, Hospitality District, IGI Airport",
      "addressLocality": "Aerocity",
      "addressRegion": "New Delhi",
      "postalCode": "110037",
      "addressCountry": "IN"
  },
  geo: {
      "latitude": 28.5535,
      "longitude": 77.1213
  },
  phone: '+91-9996265679',
  phoneDisplay: '+91 99962 65679',
  whatsapp: '919996265679',
  email: 'contact@aerocityescortservice.site',
  address: 'Asset Area 4, Hospitality District, IGI Airport, Aerocity, New Delhi 110037, India',
  url: 'https://aerocityescortservice.site',
  domain: 'aerocityescortservice.site',
  tagline: 'Escort Service in Aerocity New Delhi | 5-Star Hotel VIP Call Girls',
  description: 'Premier escort service in Aerocity New Delhi. Providing 100% verified VIP call girls, Russian models, and high-profile escorts with discreet 15-20 min hotel outcalls to JW Marriott, Pullman, Andaz, Novotel, and Roseate House.',
  keywords: [
    'aerocity escort service',
    'escort service in aerocity',
    'aerocity escort services',
    'escorts service in aerocity',
    'aerocity call girls',
    'call girls in aerocity',
    'call girls aerocity',
    'aerocity escorts',
    'escorts in aerocity',
    'aerocity hotel escorts',
    '5 star hotel escorts aerocity',
    'luxury escort service aerocity',
    'vip escorts aerocity new delhi',
    'russian escorts in aerocity',
    'russian call girls aerocity',
    'delhi airport escorts',
    'igi airport escort service',
    'airport layover escorts delhi',
    'jw marriott aerocity escorts',
    'andaz delhi escort service',
    'pullman aerocity call girls',
    'roseate house escorts aerocity',
    'novotel aerocity call girls',
    'alof aerocity escorts',
    'holiday inn aerocity escorts',
    'mahipalpur call girls',
    'mahipalpur escort service',
    'escorts near igi airport terminal 3',
    'independent escorts in aerocity',
    'high profile call girls aerocity',
    'celebrity escorts aerocity',
    'college call girls in aerocity',
    'cash on delivery escorts aerocity',
    'cod call girls aerocity',
    'outcall escort service aerocity',
    'aerocity escort agency',
    'top escort agency aerocity new delhi',
    '24/7 aerocity escorts',
    'verified call girls aerocity',
    'discrete escorts aerocity delhi',
  ],
  socialLinks: [
    { name: 'Instagram', url: 'https://instagram.com/alinavip', icon: 'Instagram' },
    { name: 'Twitter', url: 'https://twitter.com/gurgaonescort', icon: 'Twitter' },
    { name: 'Facebook', url: 'https://facebook.com/alinavip', icon: 'Facebook' },
  ],
};

export function getAlternateLanguages(path: string = '') {
  const cleanPath = path ? (path.startsWith('/') ? path : `/${path}`) : '';
  return {
    'en': `https://aerocityescortservice.site${cleanPath}`,
  };
}

export const generalFaqs = [
  {
    question: 'What is Aerocity Escort Service?',
    answer: 'Aerocity Escort Service by ALINA VIP is a premier luxury escort service agency based in the Hospitality District of Aerocity, New Delhi. We cater to corporate executives, VIP transit travelers, and luxury hotel guests with verified VIP escorts.',
  },
  {
    question: 'How fast can an escort arrive at Aerocity 5-star hotels?',
    answer: 'Our average arrival time is 15-20 minutes across all premier Aerocity hotels including JW Marriott, Andaz Delhi, Pullman New Delhi Aerocity, Novotel, Roseate House, Aloft, Holiday Inn, and Pride Plaza.',
  },
  {
    question: 'Can I book an escort during a flight layover at Delhi Airport (IGI)?',
    answer: 'Yes! Aerocity is directly connected to IGI Airport Terminals 1, 2, and 3 via the Airport Express Metro and shuttle services. We specialize in swift, discreet escort service for travelers on international and domestic layovers.',
  },
  {
    question: 'Do you offer Cash on Delivery (COD) in Aerocity?',
    answer: 'Yes. We operate with 100% transparency: you can meet and verify your escort in the comfort and privacy of your hotel room before settling the service fee in cash.',
  },
  {
    question: 'Are all profile photos 100% genuine and verified?',
    answer: 'Every profile published on our portal undergoes thorough in-person photo and background verification. We guarantee zero-catfish dispatch — the escort who arrives matches the profile you selected.',
  },
  {
    question: 'How is client privacy maintained for hotel outcalls?',
    answer: 'Our VIP escorts dress in refined, elegant executive attire that blends seamlessly into upscale hotel lobbies and luxury suites. All WhatsApp booking chats and numbers are purged immediately after the appointment.',
  },
];
